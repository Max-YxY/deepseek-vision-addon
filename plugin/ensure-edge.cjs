...
// 登录态目录：包目录可写则放包内（本地开发），否则放用户目录（npm 全局安装时）
const fs = require('fs')
const os = require('os')
const profileDir = (() => {
  try {
    ...
    return process.env.VISION_CHAIN_HOME || path.join(os.homedir(), '.dsh-vision')
  }
})()
const PROFILE = path.join(profileDir, 'edge-profile-login')
